...
//!
//! Registers handlers for `SIGTERM`, `SIGINT` (Ctrl+C), and `SIGQUIT`
//! (Ctrl+\) that atomically set a shared `stop_flag` to `true`. The
//! orchestrator's main loop polls this flag every iteration and breaks
//! cleanly — dropping `CpalSink` (with fade-out), `LibinputSource`, and
...
/// `SIGTERM`, `SIGINT` (Ctrl+C), and `SIGQUIT` (Ctrl+\).
///
...
/// IPC "stop" command, just not on Ctrl+C / `kill`).
